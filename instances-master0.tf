...
resource "aws_instance" "master0" {
  ...
  ebs_block_device = { "device_name" = "/dev/sdf", "volume_type" = "gp2", "volume_size" = "50", "delete_on_termination" = true, "encrypted" = true }
  ebs_block_device = { "device_name" = "/dev/sdg", "volume_type" = "gp2", "volume_size" = "50", "delete_on_termination" = true, "encrypted" = true }
  ebs_block_device = { "device_name" = "/dev/sdh", "volume_type" = "gp2", "volume_size" = "15", "delete_on_termination" = true, "encrypted" = true }
  user_data = "${file("userdata-master")}"
  depends_on = ["aws_placement_group.cluster-1", "aws_route.priv", "null_resource.hadoop-deployment-bash", "aws_instance.jumphost"]

  tags {
    Name = "${var.tag_name}-master0"
    type = "master"
    owner = "${var.tag_owner}"
...
#####
output "ec2_instance.master0.priv" {
  value = "${aws_instance.master0.private_dns}"
}
