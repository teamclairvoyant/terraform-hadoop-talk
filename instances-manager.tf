...
resource "aws_instance" "manager" {
  ...
  vpc_security_group_ids = ["${aws_security_group.SSH.id}", "${aws_security_group.SCM.id}"]
  ...
  ebs_block_device = { "device_name" = "/dev/sdf", "volume_type" = "gp2", "volume_size" = "200", "delete_on_termination" = true, "encrypted" = true }
  user_data = "${file("userdata-manager")}"
  depends_on = ["aws_route.priv", "null_resource.hadoop-deployment-bash", "aws_instance.jumphost"]
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo bash /home/${var.user}/hadoop-deployment-bash/install_clouderamanagerserver.sh embedded ${var.cmver}",
...
#####
output "ec2_instance.manager.priv" {
